...
//may get different pfn as the kernel might write something to the frame while forking and due to copy-on-write the virtual address 
    //of child will get mapped to different PFN than parent
  ...
  if(fork()==0)
  {
    ...
    pid_t pid=getpid();
     printf("Virtual address in %d is %p\n",pid,&mem);
      ...
      uintptr_t  paddr = 0;
      if (virt_to_phys_user(&paddr, pid, &mem))
       {
